# EC-STF, with IB
# Current addition: 1D mossy population but disabled
...
def plot_precession_and_currents(ax, egnidx, t, traj_x, theta_phase, theta_phase_plot, tidxsp_eg, Itotal_pop, Isyn_pop,
...
def plot_sequences(SpikeDF, BehDF, ActivityData, NeuronDF, MetaData, egnidx, popnidxs, fig, ax, axbig, tag):
    ...
    before_tidxsp_eg = tidxsp_eg[tidxsp_eg < thalf_sepidx]
    after_tidxsp_eg = tidxsp_eg[tidxsp_eg >= thalf_sepidx] - thalf_sepidx
    plot_tag_same = '%s Same'%tag
    info_same = plot_precession_and_currents(ax[:, 0], egnidx, t[before_idxs], traj_x[before_idxs], theta_phase[before_idxs],
                                             theta_phase_plot[before_idxs], before_tidxsp_eg, Itotal_pop[before_idxs],
    ...
    info_opp = plot_precession_and_currents(ax[:, 1], egnidx, t[after_idxs], traj_x[after_idxs], theta_phase[after_idxs],
                                            theta_phase_plot[after_idxs], after_tidxsp_eg, Itotal_pop[after_idxs],
...
def params_search(save_pth):
    ...
    wmax_ca3mos, wsd_ca3mos = 0, 2
    wmax_mosca3, wsd_mosca3 = 0, 2
    wmax_mosmos, wsd_mosmos = 0, 2
    wmax_allin, wmax_inall, wmax_inin = 3, 7.5, 0  # CH: 3, 7.5, IB: 3, 7.5
    ...
    w_mosca3 = gaufunc(xxtun1d_mos.reshape(1, nn_mos), xxtun1d_ca3.reshape(nn_ca3, 1) - mos_shift, wsd_mosca3, wmax_mosca3)
    ...
    best_nidx = get_nidx_1d_np(x=trajx_max/2, a=0, xtun=xxtun1d_ca3, atun=aatun1d_ca3)  # best angle
    best_popnidxs = np.arange(100)
    worst_nidx = get_nidx_1d_np(x=trajx_max/2, a=np.pi, xtun=xxtun1d_ca3, atun=aatun1d_ca3)  # worst angle
    worst_popnidxs = np.arange(100, nn_ca3)

    fig, ax = plt.subplots(6, 7, figsize=(14, 10), facecolor='white', constrained_layout=True)
    gs = ax[0, 0].get_gridspec()
    for axeach in ax[0:3, 0:5].ravel():
        axeach.remove()
    axbig_best = fig.add_subplot(gs[0:3, 0:5])

    gs2 = ax[3, 0].get_gridspec()
    for axeach in ax[3:, 0:5].ravel():
        axeach.remove()
    axbig_worst = fig.add_subplot(gs2[3:, 0:5])


    data_best = plot_sequences(SpikeDF, BehDF, ActivityData, NeuronDF, MetaData,
                               best_nidx, best_popnidxs, fig, ax[0:3, 5:7], axbig_best, tag='Best')
    data_worst = plot_sequences(SpikeDF, BehDF, ActivityData, NeuronDF, MetaData,
                                worst_nidx, worst_popnidxs, fig, ax[3:, 5:7], axbig_worst, tag='Worst')

    stat_dict_all = {**data_best, **data_worst}
...
save_dir = 'plots/Mossy'
os.makedirs(save_dir, exist_ok=True)
save_pth = join(save_dir, 'NO_Mossy.png')
