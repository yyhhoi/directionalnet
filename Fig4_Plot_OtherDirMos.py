...
ax_45 = [
    ...
    fig.add_axes([ax_w * 1 + wgap/2, 1 - ax_h + hgap/2, ax_w * 1 - wgap, ax_h - hgap]),
    fig.add_axes([ax_w * 2 + wgap/2 + xshift_r0134c2, 1 - ax_h + hgap/2, ax_w * 1 - wgap, ax_h - hgap]),
    fig.add_axes([ax_w * 3 + wgap/2, 1 - ax_h + hgap/2, ax_w * 1 - wgap, ax_h - hgap])
]

ax_135 = [
    fig.add_axes([ax_w * 0 + wgap/2, 1 - ax_h * 2 + hgap/2 + yshift_r14, ax_w * 1 - wgap, ax_h - hgap]),
    fig.add_axes([ax_w * 1 + wgap/2, 1 - ax_h * 2 + hgap/2 + yshift_r14, ax_w * 1 - wgap, ax_h - hgap]),
    fig.add_axes([ax_w * 2 + wgap/2 + xshift_r0134c2, 1 - ax_h * 2 + hgap/2 + yshift_r14, ax_w * 1 - wgap, ax_h - hgap]),
    fig.add_axes([ax_w * 3 + wgap/2, 1 - ax_h * 2 + hgap/2 + yshift_r14, ax_w * 1 - wgap, ax_h - hgap])
]

ax_exin1 = [
    fig.add_axes([ax_w * 0 + wgap/2+xgap_exin, 1 - ax_h * 3 + hgap/2 + yshift_r25, ax_w * 1 - wgap, ax_h - hgap]),
    fig.add_axes([ax_w * 1 + wgap/2-xgap_exin, 1 - ax_h * 3 + hgap/2 + yshift_r25, ax_w * 1 - wgap, ax_h - hgap]),
    fig.add_axes([ax_w * 2 + wgap/2+xshift_cdf, 1 - ax_h * 3 + hgap/2 + yshift_r25, ax_w * 1 - wgap, ax_h - hgap]),
    fig.add_axes([ax_w * 3 + wgap/2, 1 - ax_h * 3 + hgap/2 + yshift_r25, ax_w * 1 - wgap, ax_h - hgap]),
]

ax_90 = [
    fig.add_axes([ax_w * 0 + wgap/2, 1 - ax_h * 4 + hgap/2, ax_w * 1 - wgap, ax_h - hgap]),
    fig.add_axes([ax_w * 1 + wgap/2, 1 - ax_h * 4 + hgap/2, ax_w * 1 - wgap, ax_h - hgap]),
    fig.add_axes([ax_w * 2 + wgap/2 + xshift_r0134c2, 1 - ax_h * 4 + hgap/2, ax_w * 1 - wgap, ax_h - hgap]),
    fig.add_axes([ax_w * 3 + wgap/2, 1 - ax_h * 4 + hgap/2, ax_w * 1 - wgap, ax_h - hgap])
]


ax_270 = [
    fig.add_axes([ax_w * 0 + wgap/2, 1 - ax_h * 5 + hgap/2 + yshift_r14, ax_w * 1 - wgap, ax_h - hgap]),
    fig.add_axes([ax_w * 1 + wgap/2, 1 - ax_h * 5 + hgap/2 + yshift_r14, ax_w * 1 - wgap, ax_h - hgap]),
    fig.add_axes([ax_w * 2 + wgap/2 + xshift_r0134c2, 1 - ax_h * 5 + hgap/2 + yshift_r14, ax_w * 1 - wgap, ax_h - hgap]),
    fig.add_axes([ax_w * 3 + wgap/2, 1 - ax_h * 5 + hgap/2 + yshift_r14, ax_w * 1 - wgap, ax_h - hgap])
]

ax_exin2 = [
    fig.add_axes([ax_w * 0 + wgap/2+xgap_exin, 1 - ax_h * 6 + hgap/2 + yshift_r25, ax_w * 1 - wgap, ax_h - hgap]),
    fig.add_axes([ax_w * 1 + wgap/2-xgap_exin, 1 - ax_h * 6 + hgap/2 + yshift_r25, ax_w * 1 - wgap, ax_h - hgap]),
    fig.add_axes([ax_w * 2 + wgap/2+xshift_cdf, 1 - ax_h * 6 + hgap/2 + yshift_r25, ax_w * 1 - wgap, ax_h - hgap]),
    fig.add_axes([ax_w * 3 + wgap/2, 1 - ax_h * 6 + hgap/2 + yshift_r25, ax_w * 1 - wgap, ax_h - hgap]),
]
...
for mospairid, (mosdeg1, mosdeg2) in enumerate(mosdeg_pairs):
    ...
    for mosi, mosdeg, simdata in ((0, mosdeg1, simdata1), (1, mosdeg2, simdata2)):
        ...
        mappable2 = ax[base_axid+mosi, 3].scatter(precess_x, precess_y, c=precess_onsets, cmap='jet', marker='.', s=2, vmin=0.25*np.pi, vmax=1.25*np.pi)
        if (mosi == 0):
            ...
            cbar1.ax.tick_params(labelsize=legendsize, pad=0)
            ...
            cbar2.ax.tick_params(labelsize=legendsize, pad=0)
            cbar2.set_ticks(np.arange(0.5*np.pi, 1.1*np.pi, 0.5*np.pi))
            cbar2.set_ticks(np.arange(0.25*np.pi, 1.25*np.pi, 0.25*np.pi), minor=True)
        ...
        for jtmp in [2, 3]:
            ...
            ax[base_axid+mosi, jtmp].set_ylabel('y (cm)', labelpad=0)
    ...
    # exmu = cmean(theta_ex)
    # exmu = np.mod(exmu + np.pi, 2*np.pi) - np.pi

    diff_in = xy2_in - xy1_in
    theta_in = np.angle(diff_in[:, 0] + diff_in[:, 1] * 1j)
    # inmu = cmean(theta_in)
    # inmu = np.mod(inmu + np.pi, 2*np.pi) - np.pi

    kappa = 40
    edge_ex, den_ex = circular_density_1d(theta_ex, kappa, 100, (-np.pi/2, np.pi/2))
    edge_in, den_in = circular_density_1d(theta_in, kappa, 100, (-np.pi/2, np.pi/2))

    ax[base_axid + 2, 3].plot(edge_ex, den_ex, linewidth=0.5, color='maroon')
    ax[base_axid + 2, 3].plot(edge_in, den_in, linewidth=0.5, color='navy')
    # ax[base_axid + 2, 3].axvline(exmu, ymin=0.75, ymax=0.95, linewidth=0.5, color='maroon')
    # ax[base_axid + 2, 3].axvline(inmu, ymin=0.75, ymax=0.95, linewidth=0.5, color='navy')
    ...
    ax[base_axid, 0].axis('off')
    ax[base_axid + 1, 0].axis('off')
