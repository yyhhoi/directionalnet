...
for analysis_tag in ['', '_NoRecurrence']:
    ...
    legendsize = 8
    load_dir = join(sim_results_dir, 'fig7%s'%(analysis_tag))
    save_dir = join(plots_dir, 'fig8')
    ...
    for exin_i, exintag in enumerate(['in', 'ex']):
        ...
        simdata = load_pickle(join(load_dir, 'fig7_%s%s.pkl' % (exintag, analysis_tag)))
    ...
    fig.tight_layout()
    fig.savefig(join(save_dir, 'fig8%s.png'%analysis_tag), dpi=230)
    fig.savefig(join(save_dir, 'fig8%s.svg'%analysis_tag))