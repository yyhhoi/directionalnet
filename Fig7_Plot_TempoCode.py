...
for analysis_tag in ['', '_NoRecurrence']:
    ...
    simdata_dir = join(sim_results_dir, 'fig7%s'%(analysis_tag))
    traindata_dir = join(simdata_dir, project_tag)
    plot_dir = join(plots_dir, 'fig7')
    ...
    all_temNw = np.concatenate([np.load(join(traindata_dir, 'w_%s_ex.npy'%(project_tag))), np.load(join(traindata_dir, 'w_%s_in.npy'%(project_tag)))])
    ...
    for exin_i, exintag in enumerate(['in', 'ex']):
        ...
        simdata = load_pickle(join(simdata_dir, 'fig7_%s%s.pkl'%(exintag, analysis_tag)))
        dataset = load_pickle(join(traindata_dir, 'data_train_test_ori_%s_%s.pickle'%(project_tag, exintag)))
        temNw =  np.load(join(traindata_dir, 'w_%s_%s.npy'%(project_tag, exintag)))
        TrainResult = load_pickle(join(traindata_dir, 'TrainResult_%s_%s.pickle'% (project_tag, exintag)))
    ...
    fig.savefig(join(plot_dir, 'fig7%s.png'%(analysis_tag)), dpi=300)
    fig.savefig(join(plot_dir, 'fig7%s.pdf'%(analysis_tag)))
    fig.savefig(join(plot_dir, 'fig7%s.svg'%(analysis_tag)))
    fig_exinW2d.savefig(join(plot_dir, 'fig7_W2D%s.png'%(analysis_tag)), dpi=150)
    fig_Wcompare.savefig(join(plot_dir, 'fig7_Wcompare%s.png'%(analysis_tag)), dpi=150)