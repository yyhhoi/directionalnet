...
for analysis_tag in ['', '_NoRecurrence']:
    ...
    legendsize = 8
    load_dir = join(sim_results_dir, 'fig4%s'%analysis_tag)
    ...
    fig.savefig(join(save_dir, 'fig4%s.png'% analysis_tag), dpi=300)
    fig.savefig(join(save_dir, 'fig4%s.pdf'% analysis_tag))
    fig.savefig(join(save_dir, 'fig4%s.svg'% analysis_tag))
    fig_phase.tight_layout()
    fig_phase.savefig(join(save_dir, 'fig4_Phase%s.png'% analysis_tag))
    fig_phase.savefig(join(save_dir, 'fig4_Phase%s.svg'% analysis_tag))
