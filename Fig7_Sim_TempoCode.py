...
for analysis_tag in ['', '_NoRecurrence']:
    ...
    if analysis_tag == '_NoRecurrence':
        ...
        config_dict_WL = ph.fig6()
        config_dict_NL = ph.fig6()
    ...
    # # ============================ Simulation =====================================
    save_dir = join(sim_results_dir, 'fig7%s'%(analysis_tag))
    os.makedirs(save_dir, exist_ok=True)

    # Along the DG pathway
    save_pth = join(save_dir, 'fig7_in%s.pkl'%analysis_tag)
    print(save_pth)
    ...
    save_pth = join(save_dir, 'fig7_ex%s.pkl'%analysis_tag)
    print(save_pth)
